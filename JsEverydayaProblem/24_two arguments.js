...
Second for loop set to run across the size of the input array variable (arr), but starting at the current character (at array index i+1) being compared. This is as previous values will already have been compared so only characters later in the string from the current position need to be compared. Also +1 as starting from same position causes conflict if the element further in the array has the same value.
If the sum of value at array elements at i and j are equal to total required, set the first element to equal to the total and remove the second element at j using splice.

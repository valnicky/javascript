/*Hello guys I suggest for today another type of exercice:
...
Don’t return the matrix.Print the values in the mentioned format
...
Link with the Matrix Inverse explanation:

http://www.purplemath.com/modules/mtrxinvr.htm5*/

function printValues(mat) {
   	let stringFormat = "";
   	for (let raw = 0; raw < mat.length; raw++) {
   		for (let col = 0; col < mat.length; col++) {
   			stringFormat += mat[raw][col] + " ";
   		}
   		stringFormat += "\n";
   	}
...
function transpose(mat) {
   	for (let raw = 0; raw < mat.length; raw++) {
		mat[raw][raw] = 1; 
   		for (let col = raw + 1; col < mat.length; col++) {
   			let aux = mat[col][raw];
   			mat[col][raw] = mat[raw][col];
   			mat[raw][col] = aux;
   		}
   	}
  
   	console.log(printValues(mat));
}

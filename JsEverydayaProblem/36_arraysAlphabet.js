/*Write a function that takes two arrays of alphabetical letters as arguments (the letters are small or capital not both) and returns an array 
represents all letters in both arrays but sorted alphabetically from a-z or A-Z, the two arrays are already sorted, no need to sort them
...
Loop through twice
Check if current value is bigger than the next value - if yes assign the next value into swap variable, next index of the array is assigned the 
value of current index and current index is assigned the swap value
*/

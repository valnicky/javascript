...
If the value removed from the array in step 4 (arr[i]) summed with the current index value in modifiedArr variable is equal to a value in the input array, then return true. (NOTE: If we want to make it exclusive so that the value of sum does not equal to itself (e.g. array of [0,2,4] to return false as 0 and 2 should equal 4, not itself as 2), then the code line arr.includes(arr[i]+modifiedArr[j]) would be modifiedArr.includes(arr[i]+modifiedArr[j])) along with modifiedArr.splice(j, 1); (before the if statement check) to remove the second value being added.
...
When outside the nested for loops, it implies no matches and thus returns false.*/
 ...
 function check(arr) {
    	...
    	for (var i=0; i<arr.length; i++) {
    		...
    		for (var j=0; j<modifiedArr.length; j++) {
